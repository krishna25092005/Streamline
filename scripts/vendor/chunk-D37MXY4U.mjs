import { a as g, b as O, c as o, e as y } from "chunk-4UJWGAST.mjs";
import { j as n } from "chunk-4PTFRIXN.mjs";
var u;
((s) => {
  ...
  function r(i) {
    return typeof i != "object"
      ? !1
      : i.type === "request" || i.type === "response" || i.type === "error";
  }
  s.isMessage = r;
})((u ||= {}));
var M;
((t) => {
  ...
  class e {
    constructor(s, i, a) {
      ...
      n(this, "helper", a);
      n(this, "oneway");
      n(this, "onewayCallback");
      n(this, "iterator");
      n(this, "cancelled", !1);
      n(this, "resolveCancelPromise");
      n(this, "rejectCancelPromise");
      this.oneway = i?.oneway ?? !1;
    }
    [Symbol.asyncIterator]() {
      if (this.oneway)
        throw new o.BadRequest(
    ...
    async read(s) {
      for (
        this.iterator = this.newIterator(),
          this.oneway &&
            (this.onewayCallback = (i) => {
              if (!D(i))
                throw new o.BadResponse(
                  "StreamReader.onewayCallback received an invalid iterator result"
                );
              if (i.done) return;
              if (s(i.value))
                throw new o.BadRequest("ServiceStream callbacks cannot be async if oneway = true.");
            });
        ;
      ) {
        let i = await this.iterator.next();
        if (i.done) return;
        await s(i.value);
      }
    }
      ...
      let s = y.generateUniqueId(),
        ...
        v = async () => {
          try {
            ...
            throw ((this.cancelled = !0), h);
          }
        },
        c = async () => (
          this.cancelled ||
            (await new Promise((h, f) => {
              ((this.resolveCancelPromise = h), (this.rejectCancelPromise = f));
            })),
          i
        );
      return {
        next: async () => (this.cancelled ? i : Promise.race([v(), c()])),
        return: async () => (
          (this.cancelled = !0),
...
function D(d) {
  return d ? d.done === !0 || (d.done === !1 && d.value !== void 0) : !1;
}
var R = class {
  constructor(e) {
    n(this, "log", e);
    n(this, "expectWithoutDiscovery", (e, t) => this.addRouter(t).expectWithoutDiscovery(e));
    n(this, "discover", async (e, t, r = {}) => this.addRouter(t).discover(e, r));
    n(this, "unregister", async (e) => {
      let t = [];
...
((t) => {
  let d;
  function e() {
    return ((d = d ?? new t()), g._sharedServiceManagerIfTesting() ?? d);
  }
  t.shared = e;
})((R ||= {}));
var C = class {
    constructor(e, t) {
      n(this, "channel", e);
      n(this, "customLogger", t);
      n(this, "onMessage", (e) => {
        try {
          e.type === u.MessageType.Request
            ? e.id === u.onewayRequestId
              ? this.onOnewayRequest(e)
              : this.onRequest(e)
            : e.type === u.MessageType.Response
              ? e.method === m.method
                ? this.onDiscoveryResponse(e)
                : this.onResponse(e)
              : e.type === u.MessageType.Error
                ? this.onErrorResponse(e)
                : O(e.type, new Error(`Unknown message: ${JSON.stringify(e)}`));
        } catch (t) {
          this.log.reportError(t, { message: e });
        }
      });
      n(this, "latestDiscoveryInfo");
      n(this, "waitingDiscoveryMap", {});
      n(this, "reflectDiscoveredServices", () => {
        let e = this.latestDiscoveryInfo ? Object.keys(this.latestDiscoveryInfo.services) : [],
          t;
        this.channel.disabled &&
          ((e = Object.keys(this.waitingDiscoveryMap)), (t = new o.ServiceNotFound()));
        for (let r of e) {
          let s = this.waitingDiscoveryMap[r];
          s &&
            ((this.waitingDiscoveryMap[r] = []), s.forEach((i) => (t ? i.reject(t) : i.resolve())));
        }
      });
      n(this, "onDiscoveryResponse", (e) => {
        if (m.isValidInfo(e.body))
          ((this.latestDiscoveryInfo = e.body),
            this.reflectDiscoveredServices(),
            this.log.trace("\u2198\uFE0F Discovered services", e.body));
        else throw new o.BadResponse("Invalid discovery response", e);
        e.id !== m.broadcastMessageId && this.onResponse(e);
      });
      n(this, "broadcastDiscoveryInfo", (e) => {
        let t = {};
        for (let [s, i] of Object.entries(this.implementedServices)) {
          let a = i.service;
        ...
        try {
          this.channel.postMessage({
            type: u.MessageType.Response,
            id: e || m.broadcastMessageId,
      ...
      n(this, "onewayPromise", Promise.resolve(void 0));
      n(
        this,
        "expectWithoutDiscovery",
        (e) => (
      ...
      n(this, "discover", async (e, { timeout: t = 3e4 } = {}) => {
        ...
        return (
          t !== 1 / 0 &&
            r.push(
              E(t).then(() => {
                throw this.latestDiscoveryInfo ? new o.ServiceNotFound(e.id) : new o.TimedOut(e.id);
              })
            ),
      ...
      n(this, "throwErrorIfBadService", (e) => {
        ...
        if (r.fingerprint !== e.fingerprint)
          throw (
            ...
            new o.ServiceNotCompatible(e.id)
          );
      });
      n(this, "postOnewayRequest", (e, t) => {
        this.channel.postMessage({
          type: u.MessageType.Request,
          id: u.onewayRequestId,
          serviceId: e,
          method: t.method,
          body: t.argument,
        });
      });
      n(this, "postRequest", (e, t, r, s) => {
        if ((this.log.trace("\u2197\uFE0F", e, t), !!(this.channel.disabled ?? !1)))
          return Promise.reject(new o.ServiceNotFound(e));
        let a = {
            type: u.MessageType.Request,
            id: y.generateUniqueId(),
            serviceId: e,
            method: t.method,
            stream: T.toMessage(t.stream),
            body: t.argument,
          },
          v = y.newResolvablePromise();
        ((this.waitingRequestsMap[a.id] = { result: v, onStreamValue: s }),
        ...
        return (
          typeof r == "number" &&
            c.push(
              E(r).then(() => {
                throw new o.TimedOut();
              })
            ),
          Promise.race(c)
            .then((h) => h?.body)
            .catch((h) => {
              throw (delete this.waitingRequestsMap[a.id], h);
            })
        );
      });
      n(this, "waitingRequestsMap", {});
      n(this, "onResponse", (e) => {
        let t = e.id,
          r = t.startsWith(u.onewayStreamResponseIdPrefix);
        r && (t = t.substr(u.onewayStreamResponseIdPrefix.length));
      ...
      n(this, "onErrorResponse", (e, t) => {
        ...
        let s = t || o.reconstructErrorResponse(e.body);
        r.result.reject(s);
      });
      n(this, "implementedServices", {});
      n(this, "unregisteredServices", new Set());
      n(this, "registerImplementation", (e, t) => {
        ...
        for (let s in t.methods) {
          ...
          if (typeof a != "function")
            throw new o.Implementation(
              `Implementation for ${t.id} doesn't correctly implement ${i}()`
            );
          r[i] = a.bind(e);
        }
      ...
      n(this, "unregisterImplementation", async (e) => {
        let t = e === this.unregisterAllToken;
        t || this.log.trace("\u260E\uFE0F unregisterImplementation", e);
        let r = !1,
          s = [];
        for (let [i, a] of Object.entries(this.implementedServices))
          if (!(!t && a.rawImplementation !== e)) {
            (this.unregisteredServices.add(i), delete this.implementedServices[i], (r = !0));
            for (let [v, c] of Object.entries(this.requestedStreamsMap))
              c?.serviceId === i && s.push({ id: v, error: new o.ServiceGone(i) });
          }
        (await this.cancelStreams(s), r && this.broadcastDiscoveryInfo());
      });
      n(this, "unregisterAllToken", {});
      n(this, "unregisterAllImplementations", async () => {
        (this.log.debug("\u260E\uFE0F unregisterAllImplementations"),
          await this.unregisterImplementation(this.unregisterAllToken));
      });
      n(this, "requestedStreamsMap", {});
      n(this, "cancelStreams", async (e) => {
        let t = [];
        for (let { id: r, error: s } of e) {
          let i = this.requestedStreamsMap[r];
          (delete this.requestedStreamsMap[r], i?.iterator.throw && t.push(i.iterator.throw(s)));
        }
        await Promise.all(t);
      });
      n(this, "onRequest", async (e) => {
        if (e.method === m.method) {
          this.broadcastDiscoveryInfo(e.id);
          return;
        }
        let t = u.MessageType.Response,
          r,
          s = !1;
        try {
          let i = this.implementedServices[e.serviceId],
            a = i?.implementation;
          if (!a)
            throw this.unregisteredServices.has(e.serviceId)
              ? new o.ServiceGone(e.serviceId)
              : new o.BadRequest();
          this.log.trace("\u2198\uFE0F", e.serviceId, e);
          let v = a[e.method];
          if (!e.stream) r = await v(e.body);
          else {
            let { id: c, method: h } = T.fromMessage(e.stream),
              f = this.requestedStreamsMap[c];
            if (h === "next") {
              if (!f) {
                let p = M.isServiceStreamOptions(e.body) ? e.body : void 0;
                ((f = {
                  iterator: (await v(p))[Symbol.asyncIterator](
                    p?.oneway
                      ? (F) => (
                          this.channel.postMessage({
                            type: u.MessageType.Response,
                            id: u.onewayStreamResponseIdPrefix + e.id,
                  ...
                  (this.requestedStreamsMap[c] = f));
              }
              try {
                let p = await f.iterator.next();
                r = { done: p.done, value: p.value };
              } catch (p) {
                throw ((s = p instanceof o.ServiceGone), p);
              }
            } else if (h === "return") {
              delete this.requestedStreamsMap[c];
              let p = f?.iterator.return;
              (p && (await p()), (r = { done: !0, value: void 0 }));
            } else
              throw new o.BadRequest(
          ...
          ((t = u.MessageType.Error),
            (r = o.toMessageBody(i)),
            s || this.log.warn("\u260E\uFE0F onRequest: error", e, i));
        } finally {
          this.channel.postMessage({
            type: t,
            id: e.id,
      ...
      n(this, "testable", { waitingRequestsMap: () => this.waitingRequestsMap });
      e.addMessageListener(this.onMessage);
    }
    get log() {
      return this.customLogger ?? g.log;
    }
    destroy() {
    ...
    async waitForDiscoveryInfo(e, t = 0) {
      ...
      for (; s <= t; )
        try {
          await this.postRequest(m.serviceId, { method: m.method }, r);
          break;
        } catch (i) {
          if (!(i instanceof o.TimedOut)) throw i;
          if (!this.isWaitingForDiscovery()) break;
          s++;
        }
      if (!this.latestDiscoveryInfo) throw new o.ServiceNotFound();
    ...
    async waitForDiscoveredService(e, t) {
      let r = y.newResolvablePromise(),
        s = this.waitingDiscoveryMap[e.id] || [],
        i = this.isWaitingForDiscovery();
      return (
        (this.waitingDiscoveryMap[e.id] = s),
        s.push(r),
        i || this.waitForDiscoveryInfo(t, 2).catch(() => {}),
        this.reflectDiscoveredServices(),
        r
      );
    }
...
((s) => {
  ...
  function r(i) {
    return !!(i && typeof i == "object" && "services" in i && typeof i.services == "object");
  }
  s.isValidInfo = r;
})((m ||= {}));
var T;
((s) => {
  ...
  function t(i) {
    return i.startsWith(d)
      ? { id: i.substr(d.length), method: "return" }
      : i.startsWith(e)
        ? { id: i.substr(e.length), method: "throw" }
        : { id: i, method: "next" };
  }
  s.fromMessage = t;
  function r(i) {
...
var x = class {
    ...
    get log() {
      return g.log.extend("LocalChannel");
    }
    postMessage(e) {
      (this.log.trace("\u2197\uFE0E", e), this.listeners.forEach((t) => t(e)));
    }
  ...
  Y = new x();
var l = typeof window > "u" ? void 0 : window,
  I = "data:origin-not-initialized",
  S = class S {
    constructor(e, t) {
      n(this, "target", e);
      n(this, "trustedOrigin", I);
      n(this, "disabled");
      n(this, "listeners", new Set());
      n(this, "onMessageEvent", (e) => {
        this.log.trace(e.data, e.origin);
        let t = !1;
        if (e.source !== this.target)
          if (this === w && e.source === l && e.data?.__sourceRepresentsParentFrame) t = !0;
          else return;
        ...
        let r = e.data;
        if (u.isMessage(r)) for (let s of this.listeners) s(r);
      });
      n(this, "interceptor");
      let r = S.targetRepresentingParentFrame;
      if (e === (l ? l.parent : void 0) || e === r) {
        if (!P || w !== void 0)
          throw new Error(
    ...
    get log() {
      return g.log.extend("PostMessageChannel");
    }
    static get toParentFrame() {
      return ((P = !0), (w = w ?? new S(S.targetRepresentingParentFrame, I)), (P = !1), w);
    }
    initializeTrustedOrigin(e) {
      if (this.trustedOrigin !== I && !(this === w && e === this.trustedOrigin))
    ...
    addMessageListener(e) {
      (this.listeners.size === 0 && l?.addEventListener("message", this.onMessageEvent, !1),
        this.listeners.add(e));
    }
    removeMessageListener(e) {
      (this.listeners.delete(e),
        this.listeners.size === 0 && l?.removeEventListener("message", this.onMessageEvent, !1));
    }
...
n(
  S,
  "targetRepresentingParentFrame",
  (() => {
    let e = "__targetRepresentingParentFrame",
      t = l?.[e] ?? l?.webkit?.messageHandlers?.[e];
    return {
      disabled: !t,
      postMessage: (...r) => {
        if (l) {
...
var b = S,
  ...
  w,
  q = b.toParentFrame,
  B = l && l.opener && l !== l.opener && l.parent === l ? new b(l.opener, I) : q;
var k;
((s) => {
  function d(i, a, v, c) {
    v({ method: i, argument: a ? c : void 0, oneway: !0 });
  }
  s.onewayMethodTemplate = d;
  async function e(i, a, v, c) {
    await v({ method: i, argument: a ? c : void 0 });
  }
  s.voidMethodTemplate = e;
  async function t(i, a, v, c) {
    return await v({ method: i, argument: a ? c : void 0 });
  }
  s.valueMethodTemplate = t;
  function r(i, a, v) {
    return new M.StreamReader(i, v, a);
  }
  s.streamMethodTemplate = r;
})((k ||= {}));
export { R as a, Y as b, b as c, q as d, B as e, k as f };
//# sourceMappingURL=chunk-D37MXY4U.mjs.map
