  ...
  return (
    <>
      {showToggleButton && (
        <motion.button
          className={cn(
            `fixed z-50 text-primary cursor-pointer ${openButtonPositionClasses}`,
            btnClassName
          )}
      ...
      <AnimatePresence>
        {isOpen && (
          <div className={`fixed w-full h-full top-0 left-0 z-50 ${className}`}>
            {/* Overlay */}
            <motion.div
            ...
            {/* Drawer */}
            <motion.div
              className={`absolute h-full shadow-[8px_1px_21px_0px_rgba(17,17,26,0.3)] ${drawerPositionClasses} ${contentClassName}`}
              style={{
                backgroundColor,
                width: `${width}px`,
            ...
            >
              ...
              {showToggleButton && (
                <motion.button
                  className={cn(
                    'absolute top-4 right-6 p-2 text-slate-300 hover:text-white cursor-pointer rounded-full bg-slate-800',
                    clsBtnClassName
                  )}
                  onClick={() => setIsOpen(false)}
