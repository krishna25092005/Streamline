import { j as Ae, x as he } from "chunk-5VTIOKZ7.mjs";
import { a as te } from "chunk-SPWSWTND.mjs";
import { a as ze } from "chunk-FC7WA3BJ.mjs";
import { i as ce, m as le } from "chunk-GGY3Z3LG.mjs";
import { Oh as qe, Uh as De, X as Ee, vh as J, yi as ee } from "chunk-R3L7MKPD.mjs";
import { Lf as Oe, Ui as Le, b as ae, jf as F } from "chunk-CEQRNBNM.mjs";
import {
  E as N,
  Fd as be,
  Gd as we,
...
import { da as Pe, g as Re } from "chunk-UQ6KW42F.mjs";
import { fg as de } from "chunk-XRW2LCIK.mjs";
import { b as xe, l as _, r as Me } from "chunk-DYNCKUFC.mjs";
import { ia as Q, s as G, t as Z, u as K, x as A } from "chunk-XRJTL357.mjs";
import { a as w } from "chunk-425IX65M.mjs";
import { j as l, k as se, l as Ie } from "chunk-4PTFRIXN.mjs";
var k = Q("PartialTreeReceiver"),
  Fe = class {
    constructor(e) {
      l(this, "componentLoader", e);
      l(this, "name", "uninitialized");
      l(this, "tree");
      l(this, "chunkBuffer", new Map());
      l(this, "currentChunkId");
      l(this, "expectedTotalChunks", 0);
      l(this, "chunkTimeout");
      l(this, "chunkTimeoutMs", 3e4);
      l(this, "chunkStartTime");
      this.tree = ee.createWithRouteSegmentRoot();
    }
    clearChunkState() {
    ...
    update(e) {
      if ((De(Ee), e.name && (this.name = e.name), e.treeChunks))
      ...
      if (e.tree) {
        ...
        let t = J(e.tree.root);
        w(t, "error creating root from update message");
        let n = ee.createByAdoptingRoot(t);
        (n.loadReplicasAndCodeComponents(n.root), (this.tree = n));
      }
      if (
        (e.changes && (k.trace(this.name, "applying changes"), qe(this.tree, e.changes)), e.scopes)
      ) {
        ...
        for (let n of e.scopes) {
          let r = J(n, t);
          (w(r, "error creating scope from update message"),
      ...
      return (
        this.tree.hasUncommittedChanges() &&
          (k.trace(this.name, "commit"), (this.tree = this.tree.commitDiffs(this.componentLoader))),
        this.tree
      );
    }
      ...
      try {
        if (
          ...
          (!this.currentChunkId || this.currentChunkId !== e.chunkId) &&
            ...
            k.debug(this.name, `starting new chunked transfer: ${e.totalChunks} chunks expected`),
            (this.chunkTimeout = setTimeout(() => {
              (k.warn(this.name, `chunk timeout: clearing incomplete transfer for ${e.chunkId}`),
                this.clearChunkState());
            }, this.chunkTimeoutMs))),
          w(
          ...
          return (k.warn(this.name, `duplicate chunk received: ${e.chunkIndex}`), null);
        ...
        return (k.error(this.name, "chunk processing error:", t), this.clearChunkState(), null);
      }
      if (
        (k.debug(this.name, `received chunk ${e.chunkIndex + 1}/${e.totalChunks}`),
        this.chunkBuffer.size === e.totalChunks)
      ) {
        k.debug(this.name, "all chunks received, assembling tree");
    ...
    assembleTreeFromChunks() {
      ...
      for (let c = 0; c < this.expectedTotalChunks; c++) {
        let h = this.chunkBuffer.get(c);
        if (!h) throw new Error(`Missing chunk ${c} in chunked tree transfer`);
        if (!h.nodes || h.nodes.size === 0) throw new Error(`Chunk ${c} has no nodes`);
        if (
          (h.nodes.forEach((m, f) => {
            if (!m || !f) throw new Error(`Invalid node data in chunk ${c}`);
            e.set(f, m);
          }),
          h.childrenMap)
        )
          for (let [m, f] of h.childrenMap) t.set(m, f);
        h.rootId && (n = h.rootId);
      }
      ...
      function r(c) {
        let h = e.get(c);
        if (!h) throw new Error(`Node ${c} not found in chunks`);
        let m = t.get(c);
        return (m && m.length > 0 && (h.children = m.map(r)), h);
      }
      let o = r(n),
        i = J(o);
      w(i, "error creating root from chunked tree");
      let a = ee.createByAdoptingRoot(i);
...
var fe = [128, 512],
  ...
  Ve = class {
    constructor(e) {
      l(this, "trackThumbnailLoad", e);
      l(this, "canRenderCanvasImage", (e) => e.includes("framerusercontent.com/"));
      l(this, "zoom", 1);
      l(this, "imageStates", new Map());
      l(
        this,
        "resizeObserver",
        ...
        new IntersectionObserver(
          (e) => {
            for (let t of e) {
              let n = t.target,
                r = this.getCurrentImageState(n);
              if (!r || t.boundingClientRect.width === 0 || t.boundingClientRect.height === 0)
                continue;
              let o = 0,
                i = 0,
                a = t.isIntersecting || Ue(t);
              (a
                ? ((o = Math.round(t.boundingClientRect.width / this.zoom)),
                  (i = Math.round(t.boundingClientRect.height / this.zoom)))
                : ((o = r.width), (i = r.height)),
                !(r && r.width === o && r.height === i && r.inViewport === a) &&
      ...
      l(this, "renderCanvasImage", (e, t, n, r) => {
      ...
      l(this, "scheduled", !1);
      l(this, "scheduledRenderings", new Set());
      l(this, "processRenderMainImage", () => {
        this.scheduled = !1;
        let e = 0;
        for (let t of this.scheduledRenderings) {
          this.scheduledRenderings.delete(t);
          let n = this.imageStates.get(t);
          if (n && (this.renderImage(n), n.show !== 0 && (e++, e > Ye))) break;
        }
    ...
    getOrCreateImageState(e, t, n) {
      ...
      return r
        ? (ge(r.images[0]?.img, n, t), ge(r.images[1]?.img, n, t), ge(r.images[2]?.img, n, t), r)
        : ((r = {
            width: 0,
            height: 0,
            inViewport: !0,
            ...
            show: -1,
            images: [
              { img: pe(t, n, 0), src: "", loaded: !1 },
              { img: pe(t, n), src: "", loaded: !1 },
              { img: pe(t, n), src: "", loaded: !1 },
            ],
          }),
...
var V = new DOMMatrixReadOnly();
var X,
  ne = class {
    constructor() {
      l(this, "computedStyleCache", new Map());
      l(this, "sizeCache", new Map());
      l(this, "boundingClientRectCache", new Map());
      l(this, "matrixCache", new Map());
      l(this, "transformMatrixAndOffsetCache", new Map());
      l(this, "untransformedRectCache", new Map());
      Ie(this, X, new Map());
    }
    getComputedStyle(e) {
    ...
    hasTransforms(e) {
      return A() ? this.getCSSTransform(e) !== "none" : !this.getMatrix(e).isIdentity;
    }
    getMatrix(e) {
      let t = this.matrixCache.get(e);
      if (t) return t;
      let n = this.getCSSTransform(e);
      return ((t = n === "none" ? V : new DOMMatrix(n)), this.matrixCache.set(e, t), t);
    }
    ...
    getUntransformedRectInGroundNode(e, t, n = !0) {
      ...
      return (
        [...o, e].forEach((c) => {
          let h = this.getSize(c),
            m = this.getBoundingClientRect(c),
            f = this.getMatrix(c),
            y = n ? f.m41 / f.m44 : 0,
            v = n ? f.m42 / f.m44 : 0,
            { x: I, y: p } = this.getTransformOrigin(c),
            x = V.translate(I, p).multiplySelf(f).translateSelf(-I, -p),
            C = R.pointsAtOrigin(h),
            g = R.fromPoints(R.points(m).map((O) => t.transformPoint(O))),
            { x: Y, y: E, matrix: M } = ze(a, x, g, C),
            P = N($(Y + y), $(E + v)),
            U = N(S(P.x), S(P.y)),
            B = this.hasTransforms(c),
            H = F.hasNonTranslationTransform(f),
            b = it(c),
            { rect: z, paintDelta: L, translation: q } = rt(i, P, h, f, B, b),
            W = je(B, H, b, P, U);
          ((i.x += W.x),
            (i.y += W.y),
            (i.paintX += L.x),
            ...
            (i.translationX += q.x),
            (i.translationY += q.y),
            (a = this.getCreates3dTransformContext(c) ? M : F.flatProjectionMatrix(M)),
            this.transformMatrixAndOffsetCache.set(c, { matrix: a, offset: { ...i } }),
            this.untransformedRectCache.set(c, z),
            (u = z));
        }),
        u
      );
    ...
    getTransformAncestors(e) {
      let t = [],
        n = e.parentElement;
      for (; !Me(n) && !Be(n) && n !== e.ownerDocument?.documentElement; )
    ...
    getParentRelativeRect(e, t, n, r) {
      let o = this.getUntransformedRectInGroundNode(e, t, r),
        ...
        a,
        u,
        d = this.getOffsetParentBoundingRect(e);
      if (d) {
        let m = t.transformPoint(new DOMPoint(d.x, d.y));
        return { x: o.x - m.x, y: o.y - m.y, width: o.width, height: o.height };
      }
      let c = this.getOffsetParent(e);
      return (
        c && !Be(c)
          ? ((i = this.getUntransformedRectInGroundNode(c, t)), (a = o.x - i.x), (u = o.y - i.y))
          : n
            ? ((a = n.x), (u = n.y))
            : ((a = 0), (u = 0)),
        { x: a, y: u, width: o.width, height: o.height }
      );
    }
  };
X = new WeakMap();
function Be(s) {
  return s.classList.contains(he) || s.parentElement?.classList.contains(he);
}
function je(s, e, t, n, r) {
  return A() && s
    ? r
    : (Z() || G()) && e
...
function Ze(s) {
  let e = ie();
  if (e < 2) return Math.round($(s));
  let t = He(s) / D;
  return te(t * e, 0) / e;
}
function Ke(s) {
  return ie() < 2 ? Math.round($(s)) : Math.round($(s) * 2) / 2;
}
function Qe(s) {
  let e = He(s),
    t = 0,
...
function _e() {
  return Z() || G() ? Ke : K() ? Qe : Ze;
}
var S = _e();
function re({ x: s, y: e, width: t, height: n }) {
  let r = S(s),
    o = S(e);
  return { x: r, y: o, width: S(s + t) - r, height: S(e + n) - o };
}
function Je(s) {
  let e = ie();
  return te(s * e, 0) / e;
...
function et({ x: s, y: e, width: t, height: n }, r) {
  let { rotation: o, translation: i } = ye(r),
    a = N(s - i.x, e - i.y),
    u = re({ ...a, width: t, height: n }),
    d = N(u.x + S(t) / 2, u.y + S(n) / 2),
    c = V.translate(d.x + i.x, d.y + i.y)
      .rotateSelf(o)
      .translateSelf(-d.x, -d.y),
    h = F.convertPoint(c, R.center(u)),
    m = h.x - u.width / 2,
    f = h.y - u.height / 2;
  return {
    paintedRect: { x: m, y: f, width: u.width, height: u.height },
    snappedLayoutRect: u,
    translation: i,
  };
}
function tt({ x: s, y: e, width: t, height: n }, r, o = []) {
  let { rotation: i, translation: a } = ye(r);
  (o.includes(0) && (a.x = -0.5 * t), o.includes(1) && (a.y = -0.5 * n));
  let u = N(s - a.x, e - a.y),
    d = re({ ...u, width: t, height: n }),
    c = Math.round(t),
    h = Math.round(n),
    m = i !== 0 ? c : d.width,
    f = i !== 0 ? h : d.height,
    y = N(d.x + t / 2, d.y + n / 2),
    v = V.translate(y.x + a.x, y.y + a.y)
      .rotateSelf(i)
      .translateSelf(-y.x, -y.y),
    I = N(d.x + m / 2, d.y + f / 2),
    p = F.convertPoint(v, I),
    x = p.x - m / 2,
    C = p.y - f / 2;
  return { paintedRect: { x, y: C, width: m, height: f }, snappedLayoutRect: d, translation: a };
}
function nt(s, e, { width: t, height: n }, r) {
  let { rotation: o, translation: i } = ye(r),
    a = N(e.x - i.x, e.y - i.y),
    u = s.x + S(a.x),
    d = s.y + S(a.y),
    c = N(u + t / 2, d + n / 2),
    h = V.translate(c.x + i.x, c.y + i.y)
      .rotateSelf(o)
      .translateSelf(-c.x, -c.y),
    m = F.convertPoint(h, N(u + t / 2, d + n / 2)),
    f = m.x - t / 2,
    y = m.y - n / 2;
  return { x: f, y, width: t, height: n };
}
function rt(s, e, t, n, r, o) {
  let i = { x: 0, y: 0 },
  ...
  if (K()) {
    let y = re(d),
      v = R.offset(y, { x: u.x + a.x, y: u.y + a.y });
    if (r) {
      let { paintedRect: I, snappedLayoutRect: p, translation: x } = et(d, n),
        C = R.offset(I, { x: u.x + a.x, y: u.y + a.y }),
        g = R.offset(p, x);
      return ((i = { x: g.x - I.x, y: g.y - I.y }), { rect: C, paintDelta: i, translation: x });
    }
    return { rect: v, paintDelta: i, translation: { x: 0, y: 0 } };
  }
  if (Z() || G()) {
    let y = re(d),
      v = R.offset(y, { x: u.x + a.x, y: u.y + a.y });
    if (r) {
      let { paintedRect: I, snappedLayoutRect: p, translation: x } = tt(d, n, o),
        C = R.offset(I, { x: u.x + a.x, y: u.y + a.y }),
        g = R.offset(p, x);
      return ((i = { x: g.x - I.x, y: g.y - I.y }), { rect: C, paintDelta: i, translation: x });
    }
  ...
  let c = s.x + e.x,
    ...
    m = { x: c, y: h, ...t };
  if (r) {
    let y = R.offset({ x: s.x + S(e.x), y: s.y + S(e.y), width: m.width, height: m.height }, a),
      v = R.offset(nt(s, e, t, n), a);
    return (
      (i = { x: y.x - v.x, y: y.y - v.y }),
      { rect: v, paintDelta: i, translation: { x: 0, y: 0 } }
    );
  }
  return { rect: R.offset(m, a), paintDelta: i, translation: { x: 0, y: 0 } };
}
function ye(s) {
...
function st() {
  return A() ? 60 : G() ? 100 : 64;
}
function ot(s) {
  let [e, t] = s.split(" ").map((n) => parseFloat(n));
  if (!(!de(e) || !de(t))) return { x: e, y: t };
}
var T = Q("LayoutMeasureQueue"),
  at = new DOMMatrixReadOnly(),
  We = A() ? 1 : 0,
  Ge = class {
    constructor({
      ...
      l(this, "measureRequests", new Map());
      l(this, "measureRequestIds", new Set());
      l(this, "refinementRequests", new Map());
      l(this, "refinementRequestsByGroundNodeId", new Map());
      l(this, "richTextRenderIds", new Set());
      l(this, "previousScopeNodeId", "");
      l(this, "previousZoomLevel");
      l(this, "previousGroundNodesInViewport");
      l(this, "resizeObserver", null);
      l(this, "shouldMeasureTimerStarted", !1);
      l(this, "shouldMeasureCallback");
      l(this, "shouldMeasureCallbackDelay");
      l(this, "renderingStateTracker");
      l(this, "runShouldMeasureCallback", () => {
    ...
    add(e, t, n, r) {
      if (
        (T.trace("layout measure queue:", e),
        r && w(ce(e), "Layout templates should only have a primary rendering"),
    ...
    remeasureChildrenWithDOMRects(e) {
      if ((T.trace("remeasureChildrenWithLayout:", e.id), !!e.children && !oe(e)))
        for (let t of e.children)
          (ke(t) && !t.usesDOMRectCached()) ||
            this.remeasureCachedElements(t, (n, r) => {
              T.trace("remeasureChildrenWithLayout, child:", n, r);
            });
    }
    remeasureLayoutChildren(e) {
      if ((T.trace("remeasureLayoutChildren:", e.id), !!oe(e)))
        for (let t of e.children)
          this.remeasureCachedElements(t, (n, r) => {
            T.trace("remeasureLayoutChildren, child:", n, r);
          });
    }
    remeasureAutosizingAncestors(e, t) {
      !t ||
        !t.children ||
        (T.trace("remeasureAutosizingAncestors:", t.id),
        this.remeasureLayoutChildren(t),
        ae(t) &&
          ((t.widthType !== 2 && t.heightType !== 2) ||
            (this.remeasureCachedElements(t, (n, r) => {
              T.trace("remeasureAutosizingAncestors, add:", n, r);
            }),
    ...
    process({ scopeId: e, zoom: t, offsetX: n, offsetY: r, tree: o, groundNodesInViewport: i }) {
      this.previousScopeNodeId !== e &&
        (T.debug("switching to a new page", this.previousScopeNodeId, e),
        this.richTextRenderIds.clear(),
        this.refinementRequests.clear(),
        this.refinementRequestsByGroundNodeId.clear(),
        $e(o.get(this.previousScopeNodeId)),
        (this.previousScopeNodeId = e),
        this.resizeObserver && this.resizeObserver.disconnect(),
        (this.resizeObserver = new ResizeObserver(this.resized)));
      let a = !_(this.previousZoomLevel) && t !== this.previousZoomLevel;
      this.previousZoomLevel = t;
      let u = !_(this.previousGroundNodesInViewport) && i !== this.previousGroundNodesInViewport,
        d = new Set([...i]),
        c = new Set();
      if (!_(this.previousGroundNodesInViewport))
      ...
      if (
        ...
        (u || a) && t > We && this.processRefinementRequests(c),
        T.trace("process:", this.size()),
        this.size() === 0)
      )
        return [];
      let h = at.scale(t).translateSelf(-n, -r),
        m = [],
        f = new Map(),
        y = new ne();
      for (let [p, x] of this.measureRequests) {
        let C = o.getNode(x.nodeId);
        C &&
          (this.remeasureAutosizingAncestors(o, o.getNodeParent(C)),
          this.remeasureChildrenWithDOMRects(C),
          this.remeasureLayoutChildren(C));
      }
      let v = dt(this.measureRequests.values(), o, h),
        I = new Set();
      for (let [p, x] of this.measureRequests) {
        let { nodeId: C, renderId: g, children: Y, layoutTemplateSelector: E } = x,
          M = o.getNode(C);
        if (!M) continue;
        if (!p.isConnected) {
          T.trace("measureRequest: not connected", g);
          continue;
        }
        E
          ? (w(ce(g), "Layout templates should only have a primary rendering"),
            (M.cache.elementByLayoutTemplateSelector ??= new Map()),
            M.cache.elementByLayoutTemplateSelector.set(E, p))
          : ((M.cache.elementByRenderId ??= new Map()), M.cache.elementByRenderId.set(g, p));
        let P = v.get(C),
          U = P.matrix,
          B = P.rect;
        if (!i.has(P.id)) {
          (this.addRefinementRequest(g, { element: p, groundNodeId: P.id, measureRequest: x }),
            T.trace("measureRequest: refine", g));
          continue;
        }
        let H;
        if (Y) {
          ...
          for (let L of Y) {
            ...
            if (
              (O ||
                ((O = y.getParentRelativeRect(L, U, B)),
                f.set(L, O),
                q && m.push({ nodeId: W, renderId: q, layoutMetrics: O })),
              q)
            ) {
              let j = o.getNode(W);
              if (
                j &&
                (R.equals(j.cache.getRawDOMRect(g), O) || I.add(q),
                j.cache.setRawDOMRect(g, O),
                !Oe(j, M))
              )
                continue;
            }
        ...
        let b = f.get(p);
        (b
          ? T.trace("measureRequest: cached", g, b)
          : ((b = y.getParentRelativeRect(p, U, B, !E)),
            f.set(p, b),
            T.trace("measureRequest: getParentRelativeRect", g, b)),
          t <= We &&
            this.addRefinementRequest(g, { element: p, groundNodeId: P.id, measureRequest: x }));
        let z = { ...b };
        (E ? Object.assign(z, y.getCachedAttributes(p)) : (z.childrenRects = H),
          M &&
            (be(M) ? this.richTextRenderIds.add(M.id) : we(M) && this.resizeObserver?.observe(p),
            !E &&
              !R.equals(M.cache.getRawDOMRect(g) ?? null, b) &&
              (I.add(g), M.cache.setRawDOMRect(g, b))),
          m.push({ nodeId: C, renderId: g, layoutTemplateSelector: E, layoutMetrics: z }));
      }
      return (
        ...
        if (!(!n || n.size === 0)) {
          for (let r of n) {
            ...
            this.measureRequestIds.has(o) ||
              (T.trace("refine:", r.measureRequest.renderId),
...
function dt(s, e, t) {
  ...
  for (let { nodeId: o } of s) {
    ...
    if (d && ae(d)) {
      ...
      for (let h of u) r.set(h, d.id);
    } else i = i.inverse();
    n.set(o, { id: d?.id, matrix: i, rect: d ? Le(d) : null });
  }
  return n;
}
function $e(s) {
  ...
  let e = s.children;
  if (e) for (let t of e) $e(t);
}
function sn(s) {
  return s.resultKeyPath.length > 0 && Te(s);
}
function Xe(s, e) {
  let t = e.value;
  if (
    (Se(t) &&
      (w(!ve(t), "Computed values should not be possible in fetch fallback"),
...
function on(s, e, t) {
  ...
  try {
    ...
    return e.errorControlProp ? Xe(s, e.errorControlProp) : n;
  }
}
export { Fe as a, Ve as b, sn as c, Xe as d, on as e, Ge as f };
//# sourceMappingURL=chunk-6WDQ6B56.mjs.map
